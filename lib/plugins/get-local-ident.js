...
module.exports = function getLocalIdent(
  ...
  let className = loaderUtils
    .interpolateName(
      context,
      isModule ? `${fileNameOrFolder}|${localName}` : `${localName}`,
      options
    )
    .replace(/\.module/, '') // remove '.module' from the classname

  // Find the selector name and the module name
  let match = className.match(/^([a-z0-9-_]+)+\|*([a-z0-9-_]+)*$/i)
  ...
  switch (selectorName) {
    ...
    case 'repitem':
    case 'repcontrol_add':
    case 'repcontrol_edit':
    case 'repcontrol_move':
    case 'repcontrol_del':
    case 'repcontrol':
    case 'itemcontrol':
    case 'btn':
    case 'inlinerollresult':
    case 'fullfail':
    case 'fullcrit':
    ...
    default:
      className =
        isModule && moduleName
          ? `${camelCase(moduleName)}-${selectorName}`
          : `${selectorName}`
  }
